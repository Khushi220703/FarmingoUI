...
const CropTools = () => {
  ...
  return (
    <div className="crop-tools-container">
      ...
      {/* Modals */}
      {/* {isChemicalOpen && <ChemicalModal onClose={() => setIsChemicalOpen(false)} />} */}
      {isFertilizerOpen && <FertilizerModal onClose={() => setIsFertilizerOpen(false)} />}
      {isPriceOpen && <PriceModal onClose={() => setIsPriceOpen(false)} />}
    </div>
  );
};
